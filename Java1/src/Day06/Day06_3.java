			...
			if( ch == 1 ) {  // 회원가입 
				...
				for(Member temp : memberlist) {
					if(temp != null && temp.name.equals(name) && temp.phone.equals(phone)) {
						...
						System.out.println("고객님의 아이디는: " + temp.id);
						findcheck = false;
						break;
					}
				}
				if (findcheck) System.err.println("알림)) 동일한 회원정보가 없습니다.");
				...
				for(Member temp : memberlist) {
					if(temp != null && temp.id.equals(id) && temp.phone.equals(phone)) {
						...
						System.out.println("고객님의 비밀번호는: " + temp.password);
						findcheck = false;
						break;
					}
				}
