...
public class Member {
	...
	boolean 회원가입() {
		...
		System.out.print(" 아이디 : ");	String id = Day07_5.scanner.next();
		System.out.print(" 비밀번호 : ");	String password = Day07_5.scanner.next();
		System.out.print(" 이름 : ");	String name = Day07_5.scanner.next();
		System.out.print(" 연락처 : ");	String phone = Day07_5.scanner.next();
			...
			for( Member temp : Day07_5.members ) {
				if( temp == null ) { // null 찾기 [ null 찾았을때 null-1 하면 마지막회원 ]
					if( j == 0 ) {   // 첫번째 인덱스[0번인덱스]가 null => 첫회원 => 첫회원번호 = 1 부여
						...
						bno = Day07_5.members[ j-1 ].mno + 1 ; // null 앞 [ 마지막회원 ]번호에 +1
						break; // 끝내기 
					}
				}
				j++; // 인덱스 증가
		...
		for( Member temp : Day07_5.members ) {
			if( temp == null ) { // 빈공간이면 
				Day07_5.members[i] = member; // 빈 인덱스에 새로운 회원 대입
