...
public class Server{	// fxml 사용하지 않는 클래스 [ 서버 컨트롤 사용 ] 
		...
		public void send( String msg ) { 
			Runnable runnable = new Runnable() {
				...
				public void run() {
					...
					}catch( Exception e ) { System.out.println("서버가 메시지 보내기 실패:"+e); serverstop();}
				}
			}; // 멀티스레드  구현 끝 
			threadpool.submit(runnable); // 해당 멀티스레드를 스레드풀에 넣어주기 
		}
    ...
    public void serverstart( String ip , int port) {  
    	...
    	}catch( Exception e ) { System.out.println("서버 생성 실패:"+e); serverstop();}
    	...
    	Runnable runnable = new Runnable() {
			...
			public void run() {
				...
				}catch( Exception e ) { System.out.println("서버가 클라이언트 연결실패 :" + e);  serverstop(); }
			}
		};// 멀티스레드 구현 끝
    ...
    public void serverstop() { 
    	try {
	    	...
	    	// 3. 스레드풀 닫기
	    	threadpool.shutdown();
    	}catch( Exception e ) {System.out.println("아아");  serverstop(); }
    }
